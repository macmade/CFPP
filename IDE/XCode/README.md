...
`CF::UUID` is a wrapper for `CFUUIDRef`.

**Creation:**

`CF::UUID` can be instantiated using another `CF::UUID` or `CFUUIDRef`.  
In addition, the parameter-less constructor will create a new UUID: 

    CF::UUID uuid1; /* Creates a new UUID */
    CF::UUID uuid2 = uuid1;
    CF::UUID uuid3 = someCFUUIDRefObject;

**Equality:**

CF::UUID can be compared to another CF::UUID, a CFUUIDRef or a std::string:

    CF::UUID uuid1;
    CF::UUID uuid2;
    
    if( uuid1 == uuid2 )                                                 { /* ... */ }
    if( uuid1 != someCFUUIDRefObject )                                   { /* ... */ }
    if( uuid1 == std::string( "F6988C82-3896-47E4-822B-F0EDF9C326E7" ) ) { /* ... */ }

**Casting:**

`CF::UUID` can be safely casted to a `CFUUIDRef` or a `std::string`:

    CF::UUID    uuid;
    CFURLRef    cfUUID = uuid;
    std::string s      = uuid;
